...
function _decode(jpegData, userOpts = {}) {
  var defaultOpts = {
...
function loadJpeg(path) {
  const data = _decode(fs.readFileSync(path))
  return {
    data: data.data,
    height: data.height,
    width: data.width
